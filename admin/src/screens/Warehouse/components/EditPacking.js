...
export default function ReleasePacking({ open, setOpen, matirialData }) {
  const { register, resetField } = useForm();
  const [addMatirial] = api.useAddMatirialMutation();
  const [editMatirial] = api.useEditMatirialMutation();
  ...
  const handleAmount = (event) => {
    ...
    const displayedAmount = parseInt(matirialData?.amount || formData.amount);
    setFormData({
      ...formData,
      updatedAmount: enteredAmount,
      amount: displayedAmount - enteredAmount,
    });
  };
    ...
    <div>
      ...
      >
        <form id="form" onSubmit={handleSubmit}>
          <Box sx={boxSX}>
            <Typography sx={titleSx}>Realease Packing Material</Typography>
